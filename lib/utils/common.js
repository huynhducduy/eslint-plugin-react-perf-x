...
function createRule(description, errorMessage, isViolation) {
  return {
    ...
    create: function (context) {
      ...
      const ignoreSourcePrefixes = []; // Separate array for prefix matching
      ...
      if (ignoreSources) {
        for (let i = 0; i < ignoreSources.length; i++) {
          ...
          if (typeof config === "string") {
            ...
            ignoreSourcePrefixes.push([config + "/", true]);
          } else {
            ...
            ignoreSourcePrefixes.push([source + "/", currentSet]);
          }
        }
      }
        ...
        JSXAttribute: function (node) {
          ...
          if (ignoreSources) {
            ...
            if (parentNameType === "JSXMemberExpression") {
              ...
              const propertyName = parentName.property.name;
            ...
            if (source) {
              ...
              for (let i = 0; i < ignoreSourcePrefixes.length; i++) {
                const [prefix, ignoreImportNames] = ignoreSourcePrefixes[i];
