      ...
      return {
        ...
        JSXAttribute: function (node) {
          ...
          if (node.parent.name.type === "JSXIdentifier") {
            ...
            componentName = node.parent.name.object.name;
          }
          ...
          if (ignoreSources) {
            ...
            if (node.parent.name.type === "JSXMemberExpression") {
              ...
              const objectSource = sourceMap.get(object);

              if (objectSource) {
                // Check both:
                // 1. The imported name of the object (e.g., Item in <Item.Tag />)
                // 2. The property name (e.g., Item in <Foo.Item /> where Foo is default import)
                if (
                  ignoreSourcesSet.has(objectSource[0]) ||
                  ignoreSourcesSet.has(
                    hashSourceAndImportName(objectSource[0], objectSource[1])
                  ) ||
                  ignoreSourcesSet.has(
                    hashSourceAndImportName(objectSource[0], property)
                  )
                ) {
                  return;
                }
